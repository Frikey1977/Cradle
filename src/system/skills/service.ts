...
export async function getSkillFileTree(slug: string): Promise<FileTreeNode[]> {
  const installPath = getSkillInstallPath(slug);
  console.log(`[Skills] File tree - SKILLS_DIR: ${SKILLS_DIR}`);
  console.log(`[Skills] File tree - installPath: ${installPath}`);

  try {
    await access(installPath);
    console.log(`[Skills] File tree - directory exists`);
  } catch (e) {
    // 目录不存在返回空数组
    console.log(`[Skills] File tree - directory NOT found: ${installPath}`);
    return [];
  }

  const children = await readDirRecursive(installPath, installPath);
  console.log(`[Skills] File tree - children count: ${children.length}`);
