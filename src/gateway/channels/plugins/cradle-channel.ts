...
export class CradleChannel extends BaseChannelPlugin {
  ...
  private async handleAuth(
    ...
    try {
      ...
      if (employeeId) {
        try {
          const contactResult = await query<
            Array<{ sid: string }>
          >(
            `SELECT sid FROM t_contacts WHERE source_id = ? AND type = 'employee' AND deleted = 0`,
            [employeeId]
          );
          if (contactResult.length > 0) {
            contactId = contactResult[0].sid;
            this.logger.info(
              `Found contactId for employee ${employeeId}: ${contactId}`
            );
          }
        } catch (err) {
          this.logger.warn(`Failed to query contactId: ${err}`);
        }
      }
      ...
      client.contactId = contactId; // 设置 contactId，用于接收心跳消息等

      this.logger.info(
        `Client ${clientId} authenticated: userId=${userId}, employeeId=${employeeId}, contactId=${contactId}, channelName=${channelUserName}`
      );
  ...
  private handleChatMessage(
    ...
    // 调试日志 - 记录客户端ID信息
    this.logger.info(`[handleChatMessage] Client info: clientId=${client.id}, userId=${client.userId}, userName=${client.userName}, contactId=${client.contactId}, employeeId=${client.employeeId}`);
  ...
  async sendMessage(context: OutboundMessageContext): Promise<void> {
    ...
    this.logger.info(`[sendMessage] Looking for client with targetId: ${targetId}, total clients: ${this.clients.size}`);
    // 打印所有客户端的信息用于调试
    for (const c of this.clients.values()) {
      this.logger.info(`[sendMessage] Client: id=${c.id}, userId=${c.userId}, userName=${c.userName}, contactId=${c.contactId}, isAuthenticated=${c.isAuthenticated}`);
    }
    ...
    if (client) {
      this.logger.info(`[sendMessage] Found client by clientId: ${client.id}`);
    }
    ...
    if (!client) {
      for (const c of this.clients.values()) {
        if (c.userId === targetId || 
          ...
          client = c;
          this.logger.info(`[sendMessage] Found client by userId/userName/contactId: ${client.id}`);
          break;
        }
      }
    }
    ...
    if (!client) {
      this.logger.warn(`[sendMessage] Client ${targetId} not found`);
      return;
    }
    ...
    // 获取发送者信息 - 从 originalContext 获取
    // senderId 是发送者的 ID（Agent ID），senderName 是发送者的名称
    this.logger.info(`[sendMessage] context.originalContext: ${JSON.stringify(context.originalContext)}`);
    const senderId = context.originalContext?.senderId || "agent";
    const senderName = context.originalContext?.senderName || "Agent";
    this.logger.info(`[sendMessage] Extracted senderId: ${senderId}, senderName: ${senderName}`);
    ...
    try {
      ...
      const payload: any = {
        id: messageId,
        content: content,
        sender: isSystemMessage ? "System" : senderName,
        senderId: senderId,
        timestamp: Date.now(),
        replyTo: context.replyToId || null, // 确保 replyTo 始终存在（即使是 null）
      };
      ...
      this.logger.info(`[sendMessage] Sending message to client ${client.id}: type=${messageType}, contentLength=${payload.content?.length || 0}`);
      this.logger.info(`[sendMessage] Message JSON: ${messageJson.substring(0, 500)}`);
      client.socket.send(messageJson);
      this.logger.info(`[sendMessage] Message sent successfully to client ${client.id}`);
    } catch (error) {
