...
export default async function employeeRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: CreateEmployeeDto }>(
    ...
    async (request: FastifyRequest<{ Body: CreateEmployeeDto }>, reply: FastifyReply) => {
      try {
        ...
        // 检查英文名是否已存在
        if (data.eName) {
          const exists = await isENameExists(data.eName);
          if (exists) {
            return validationErrorResponse(reply, "英文名已存在");
          }
        }
  ...
  fastify.put<{ Params: { id: string }; Body: UpdateEmployeeDto }>(
    ...
    async (
      ...
      // 检查英文名是否已存在（排除当前记录）
      if (data.eName && data.eName !== existingEmployee.eName) {
        const exists = await isENameExists(data.eName, id);
        if (exists) {
          return validationErrorResponse(reply, "英文名已存在");
        }
      }
  ...
  // 检查英文名是否存在
  fastify.get<{ Querystring: ENameExistsQuery }>(
    "/e-name-exists",
    async (
      request: FastifyRequest<{ Querystring: ENameExistsQuery }>,
      reply: FastifyReply,
    ) => {
      const { eName, id } = request.query;

      if (!eName) {
        return successResponse(reply, false, "检查完成");
      }

      const exists = await isENameExists(eName, id);
      return successResponse(reply, exists, "检查完成");
    },
  );
}
