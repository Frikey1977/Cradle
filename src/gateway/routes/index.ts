...
import followupRoutes from "./followups.js";
...
import systemLogsRoutes from "./system-logs.js";
...
export async function registerRoutes(fastify: FastifyInstance) {
  ...
  // 所有 API 路由添加 /api 前缀
  await fastify.register(async (apiApp) => {
    await apiApp.register(authRoutes, { prefix: "/auth" });

    // 注册 /users/info 路由（需要认证）
    await apiApp.register(async (app) => {
    ...
    await apiApp.register(async (app) => {
      ...
      await app.register(systemLogsRoutes, { prefix: "/llm/logs" });
      await app.register(timezoneRoutes, { prefix: "" });
      await app.register(customerRoutes, { prefix: "/customer/customers" });
      await app.register(opportunityRoutes, { prefix: "/customer/opportunities" });
      await app.register(dealRoutes, { prefix: "/customer/deals" });
      await app.register(followupRoutes, { prefix: "/customer/followups" });
    });
  }, { prefix: "/api" });
}
