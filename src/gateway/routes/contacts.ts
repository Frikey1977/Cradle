...
export default async function contactRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Params: { sid: string } }>, reply: FastifyReply) => {
      try {
        ...
        return successResponse(reply, parsedProfile);
      } catch (error: any) {
        logError(request.log, error);
  ...
  // 更新联系人语言设置（转发到 Gateway Master）
  fastify.post(
    "/:sid/language",
    async (
      request: FastifyRequest<{ Params: { sid: string }; Body: { language: string; agentId: string } }>,
      reply: FastifyReply,
    ) => {
      try {
        const { sid } = request.params;
        const { language, agentId } = request.body;

        if (!language || !agentId) {
          return validationErrorResponse(reply, "语言和 Agent ID 不能为空");
        }

        const response = await fetch(`http://localhost:3000/api/contact/${sid}/language`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ language, agentId }),
        });

        if (!response.ok) {
          const errorText = await response.text();
          return internalErrorResponse(reply, `Gateway Master 错误: ${errorText}`);
        }

        const result = await response.json();
        return successResponse(reply, result, "语言设置已更新");
      } catch (error: any) {
        logError(request.log, error);
        return internalErrorResponse(reply, "更新语言设置失败");
      }
    }
  );

  // 更新联系人时区设置（转发到 Gateway Master）
  fastify.post(
    "/:sid/timezone",
    async (
      request: FastifyRequest<{ Params: { sid: string }; Body: { timezone: string; agentId: string } }>,
      reply: FastifyReply,
    ) => {
      try {
        const { sid } = request.params;
        const { timezone, agentId } = request.body;

        if (!timezone || !agentId) {
          return validationErrorResponse(reply, "时区和 Agent ID 不能为空");
        }

        const response = await fetch(`http://localhost:3000/api/contact/${sid}/timezone`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ timezone, agentId }),
        });
        ...
        const result = await response.json();
        return successResponse(reply, result, "时区设置已更新");
      } catch (error: any) {
        logError(request.log, error);
        return internalErrorResponse(reply, "更新时区设置失败");
      }
    }
  );
