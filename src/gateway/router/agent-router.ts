...
export class AgentRouter {
  ...
  async route(
    ...
    // 获取用户语言偏好和 eName
    try {
      const contactProfile = await query<Array<{ profile: string | null; e_name: string | null }>>(
        `SELECT c.profile, e.e_name FROM t_contacts c 
         LEFT JOIN t_employees e ON c.source_id = e.sid 
         WHERE c.sid = ?`,
        [contactId]
      );
      if (contactProfile.length > 0) {
        const row = contactProfile[0];
        if (row.profile) {
          const profile = typeof row.profile === 'string' 
            ? JSON.parse(row.profile) 
            : row.profile;
          agentMessage.preferredLanguage = profile.preferredLanguage || profile.preferences?.language || "zh-CN";
          this.logger.info(`Contact ${contactId} preferred language: ${agentMessage.preferredLanguage}`);
        }
        if (row.e_name) {
          agentMessage.contactEName = row.e_name;
          this.logger.info(`Contact ${contactId} eName: ${agentMessage.contactEName}`);
        }
      }
    } catch (error) {
      this.logger.warn(`Failed to get contact profile: ${error}`);
    }
  ...
  private async createChannelMapping(
    ...
    try {
      ...
      const existing = await query<{ count: number }[]>(
        `SELECT COUNT(*) as count FROM r_channel_contact 
         WHERE channel_id = ? AND contact_id = ? AND sender = ?`,
