...
export class LLMClient implements LLMServiceInterface {
  ...
  handleResponse(response: LLMIPCResponse): void {
    ...
    switch (response.type) {
      ...
      case "llm:get-instance-config-response":
        // 获取实例配置响应
        pending.resolve(response.payload);
        this.pendingRequests.delete(response.requestId);
        break;
  ...
  async *streamGenerate(
    ...
    let finalResponse: StreamEndPayload | null = null;
    ...
    // 发送请求
    const responsePromise = new Promise<StreamEndPayload>((resolve, reject) => {
      this.pendingRequests.set(requestId, {
        resolve: (value) => {
          finalResponse = value as StreamEndPayload;
          resolve(value as StreamEndPayload);
        },
        reject,
        isStream: true,
        streamCallbacks,
    ...
    // 处理 tool calls
    const toolCallsData = (finalResponse as StreamEndPayload | null)?.toolCalls;
    if (toolCallsData && toolCallsData.length > 0) {
      console.log(`[LLMClient] Processing ${toolCallsData.length} tool calls from stream end`);
   ...
   * 在 Worker 进程中通过 IPC 向 Master 请求
   */
  async getInstanceConfig(instanceId: string): Promise<{ 
    ...
    return new Promise((resolve, reject) => {
      this.pendingRequests.set(requestId, {
        resolve: (value) => resolve(value.config),
        reject,
        isStream: false,
      });
    });
