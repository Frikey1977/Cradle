...
export class ProfilePromptBuilder {
   ...
   * 单独构建 Agent 画像（你的身份）
   */
  buildAgentProfile(profiles: ProfileCollection): string {
    if (!profiles.agent) {
      return "";
    }
   ...
   * 单独构建用户画像（当前用户）
   */
  buildContactProfile(profiles: ProfileCollection): string {
    if (!profiles.contact) {
      return "";
    }
    
    return this.buildContactSection(profiles.contact);
  }

  /**
   * 单独构建关系画像（双方关系）
   */
  buildRelationshipProfile(profiles: ProfileCollection): string {
    if (!profiles.relationship) {
      return "";
    }
    
    return this.buildRelationshipSection(profiles.relationship);
  }
