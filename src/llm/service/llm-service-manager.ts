...
export class LLMServiceManager {
  ...
  async getInstanceConfig(instanceId: string): Promise<{ 
    ...
    // 如果指定实例不存在，自动选择一个可用实例
    if (!instance) {
      console.warn(`[LLMServiceManager] Instance ${instanceId} not found, auto-selecting available instance...`);
      
      // 按能力筛选可用实例（默认使用 textGeneration 能力）
      const candidates = Array.from(this.instances.values()).filter(inst => {
        // 检查是否有文本生成能力
        const hasCapability = inst.capabilities.includes("textGeneration");
        if (!hasCapability) return false;
      ...
      if (candidates.length === 0) {
        console.error(`[LLMServiceManager] No available instances found`);
        return undefined;
      }
      ...
      console.log(`[LLMServiceManager] Auto-selected instance: ${instance.sid} (${instance.name})`);
    }
