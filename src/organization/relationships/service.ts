...
export async function getAgentContacts(agentId: string): Promise<AgentContactBinding[]> {
  ...
  for (const row of rows) {
    ...
    // 解析 agent_contact 获取 owner 标记
    if (row.agent_contact) {
      try {
        const agentContact = typeof row.agent_contact === "string" 
          ? JSON.parse(row.agent_contact) 
          : row.agent_contact;
        binding.owner = agentContact?.owner === true;
      } catch {
        // 忽略解析错误
      }
    }
...
export async function bindAgentContact(data: BindAgentContactDto): Promise<string> {
  ...
  if (existingDeleted.length > 0) {
    ...
    const restoreSql = `
      UPDATE ${TABLE_NAME}
      SET deleted = 0, agent_contact = ?, update_time = CURRENT_TIMESTAMP
      WHERE sid = ?
    `;
  ...
  const sql = `
    INSERT INTO ${TABLE_NAME} (sid, agent_id, contact_id, contact_agent, agent_contact, deleted, create_time, update_time)
...
export async function bindAgentToEmployee(data: BindAgentEmployeeDto): Promise<AgentContactBinding> {
  ...
  // 3. 创建 Relationship
  const relationshipId = await bindAgentContact({
...
export async function updateShortTermMemory(
  ...
  if (!existing || existing.length === 0) {
    ...
    const sid = generateUUID();
    const insertSql = `
      INSERT INTO ${TABLE_NAME} (sid, agent_id, contact_id, short_term_memory, contact_agent, agent_contact, deleted, create_time, update_time)
      VALUES (?, ?, ?, ?, ?, ?, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    `;
    await run(insertSql, [
      sid,
    ...
    const updateSql = `
      UPDATE ${TABLE_NAME}
      SET short_term_memory = ?, update_time = CURRENT_TIMESTAMP
      WHERE agent_id = ? AND contact_id = ? AND deleted = 0
    `;
