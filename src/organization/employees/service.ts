 ...
 * 检查英文名(e_name)是否存在
 */
export async function isENameExists(eName: string, excludeId?: string): Promise<boolean> {
  if (!eName || eName.trim() === "") {
    return false;
  }

  let sql = "SELECT COUNT(*) as count FROM t_employees WHERE e_name = ? AND deleted = 0";
  ...
  return withTransaction(async (connection) => {
    ...
    await connection.execute(
      `INSERT INTO t_employees (
        sid, name, e_name, employee_no, oid, position_id, type, location, email, phone, hire_date,
