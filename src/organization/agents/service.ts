...
export async function getAgentList(queryParams: AgentQuery): Promise<AgentListResult> {
  const { oid, mode, pattern, keyword, status, page = 1, pageSize = 20 } = queryParams;
...
/**
 * 根据用户ID获取绑定的Agent列表
 * 通过 user -> employee -> contact -> relationship -> agent 的关联链查询
 */
export async function getAgentsByUserId(userId: string): Promise<Agent[]> {
  const rows = await query<Agent[]>(
    `SELECT
      ...
      a.timestamp
    FROM t_agents a
    INNER JOIN t_relationship r ON r.agent_id = a.sid AND r.deleted = 0
    INNER JOIN t_contacts c ON c.sid = r.contact_id AND c.deleted = 0 AND c.type = 'employee'
    INNER JOIN t_employees e ON e.sid = c.source_id AND e.deleted = 0
    LEFT JOIN t_departments o ON a.oid = o.sid AND o.deleted = 0
    LEFT JOIN t_positions p ON a.position_id = p.sid AND p.deleted = 0
    WHERE a.deleted = 0
...
export async function createAgent(data: CreateAgentDto): Promise<string> {
  ...
  await run(
    `INSERT INTO t_agents (
      sid, name, e_name, title, agent_no, description, oid, position_id, mode, pattern,
      avatar, config, profile, soul,
 ...
 * 更新心跳配置
 */
export async function updateHeartbeatConfig(id: string, heartbeat: any): Promise<void> {
  await run(
    "UPDATE t_agents SET heartbeat = ?, timestamp = NOW() WHERE sid = ? AND deleted = 0",
    [JSON.stringify(heartbeat), id],
  );
}
 ...
 * 获取心跳配置
 */
export async function getHeartbeatConfig(id: string): Promise<any | null> {
  const rows = await query<Array<{ heartbeat: any }>>(
    "SELECT heartbeat FROM t_agents WHERE sid = ? AND deleted = 0",
    [id],
  );
  ...
  return parseJsonField(rows[0].heartbeat);
}
