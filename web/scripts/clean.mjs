 ...
 * @param {number} _departmentsh - 当前递归深度
 * @returns {Promise<boolean>} - 是否需要进一步递归处理
 */
async function processItem(currentDir, item, targets, _departmentsh) {
  // 跳过特殊目录
 ...
 * @param {string[]} targets - 要删除的目标列表
 * @param {number} departmentsh - 当前递归深度，避免过深递归
 */
async function cleanTargetsRecursively(currentDir, targets, departmentsh = 0) {
  // 限制递归深度，避免无限递归
  if (departmentsh > 10) {
    console.warn(`Max recursion departmentsh reached at: ${currentDir}`);
    return;
  }
  ...
  for (let i = 0; i < dirents.length; i += CONCURRENCY_LIMIT) {
    ...
    const tasks = batch.map(async (dirent) => {
      ...
      const shouldRecurse = await processItem(currentDir, item, targets, departmentsh);

      // 如果是目录且没有被删除，则递归处理
      if (shouldRecurse && dirent.isDirectory()) {
        const itemPath = normalize(join(currentDir, item));
        return cleanTargetsRecursively(itemPath, targets, departmentsh + 1);
      }
