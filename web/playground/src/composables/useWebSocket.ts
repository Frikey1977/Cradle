  ...
  const handleMessage = (event: MessageEvent) => {
    ...
    try {
      ...
      switch (message.type) {
        ...
        case "message":
          // 收到业务消息
          console.log("[WebSocket] STEP 1: Received message from server:", JSON.stringify(message.payload, null, 2));
          console.log("[WebSocket] STEP 2: Calling onMessage callback...");
          onMessage?.(message);
          console.log("[WebSocket] STEP 3: onMessage callback returned");
          break;

        default:
          // 其他业务消息
          console.log("[WebSocket] STEP 1: Received other message:", message);
          console.log("[WebSocket] STEP 2: Calling onMessage callback...");
          onMessage?.(message);
          console.log("[WebSocket] STEP 3: onMessage callback returned");
      }
