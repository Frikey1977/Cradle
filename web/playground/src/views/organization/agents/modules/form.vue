...
// 当前Agent的服务模式（使用pattern字段判断绑定规则）
const agentPattern = computed(() => formData.value?.pattern || '');
// 新增/编辑联系人绑定表单
...
async function loadModeOptions() {
  try {
    ...
    console.error('Failed to load mode options:', error);
  }
}

// 加载服务模式选项（从代码管理 organization.agents.pattern 获取）
...
const [BasicForm, basicFormApi] = useVbenForm({
  ...
  schema: useBasicInfoSchema(modeOptions.value, patternOptions.value, loadPositionOptionsByOid),
        ...
        <div class="p-4">
          ...
          <div class="mb-4 p-3 bg-blue-50 rounded">
            <span class="text-blue-600">
              {{ $t(`organization.agents.pattern.${agentPattern}.tip`) }}
            </span>
          </div>
          ...
          <div class="space-y-3">
            ...
            >
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2 ml-4">
                  <Button
                    v-if="agentPattern !== 'public' && agentPattern !== 'department'"
                    type="link"
                    danger
                    size="small"
            ...
            <div v-if="agentContacts.length === 0 && !agentContactsLoading" class="text-center text-gray-400 py-8">
              {{ agentPattern === 'public' || agentPattern === 'department'
