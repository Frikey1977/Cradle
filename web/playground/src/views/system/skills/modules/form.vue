...
async function loadFileTree() {
  ...
  try {
    ...
    if (tree.length > 0 && tree[0].children) {
      const skillMdNode = findSkillMdNode(tree[0].children);
      if (skillMdNode) {
        // 延迟执行，确保树已渲染
        setTimeout(() => {
          selectedFilePath.value = skillMdNode.key as string;
          loadFileContent(skillMdNode.key as string);
        }, 100);
      }
    }
  } catch (error) {
...
// 查找 SKILL.md 节点
function findSkillMdNode(nodes: TreeProps['treeData']): any {
  for (const node of nodes || []) {
    if (node.title === 'SKILL.md') {
      return node;
    }
    if (node.children) {
      const found = findSkillMdNode(node.children);
...
async function loadFileContent(filePath: string) {
  ...
  try {
    const content = await skillApi.getSkillFileContent(filePath);
