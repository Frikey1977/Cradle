...
function renderMarkdown() {
  ...
  const markdownElement = React.createElement(ReactMarkdown, {
    ...
    components: {
      // 自定义段落渲染，减少段落间距
      // 使用 div 代替 p 以避免嵌套问题（如 code 块中包含 pre 等块级元素）
      p: ({ children }: { children?: React.ReactNode }) => {
        return React.createElement('div', { style: { marginBottom: '0.25em', marginTop: 0 } }, children);
      },
      // 自定义列表渲染，减少间距
      ...
      code: ({ inline, className, children }: { inline?: boolean; className?: string; children?: React.ReactNode }) => {
        ...
        return React.createElement(React.Fragment, null,
          match ? React.createElement('div', { className: 'code-language', style: { marginBottom: '4px', fontSize: '0.85em', color: '#666' } }, match[1]) : null,
          React.createElement('pre', { className: 'code-block', style: { margin: 0 } },
