...
import { Card, Avatar, Empty, Spin, Button } from "ant-design-vue";
...
// 强制刷新计数器，用于触发 messageGroups 重新计算
...
// 消息分组：将连续的 agent/thinking/heartbeat 消息分为一组
const messageGroups = computed(() => {
  // 依赖 refreshCounter 以确保强制刷新时重新计算
  const counter = refreshCounter.value;
  
  const groups: { type: 'user' | 'agent' | 'system' | 'heartbeat'; messages: ChatMessage[] }[] = [];
  let currentGroup: { type: 'user' | 'agent' | 'system' | 'heartbeat'; messages: ChatMessage[] } | null = null;
  ...
  for (const message of props.messages) {
    if (message.type === 'system') {
      ...
      currentGroup = null;
    } else if (message.type === 'heartbeat' || message.isHeartbeat) {
      // 心跳消息单独成组（使用特殊样式）
      ...
      console.warn("[ChatArea] Unknown message type:", message.type, "for message:", message.id);
    }
  }
    ...
    >
      ...
      <template v-if="messages.length > 0">
        ...
        <div
          ...
          :class="[
            'message-group flex gap-3 relative',
        ...
        >
          ...
          <template v-else-if="group.type === 'agent'">
            ...
            <!-- 消息内容 -->
            <div class="message-content max-w-[80%] items-start">
              <!-- 发送者名称 -->
              ...
              <template v-for="message in group.messages" :key="message.id">
                ...
                <template v-else>
                  <!-- 思考中状态 -->
                  <div
                    v-if="message.isThinking && !message.content"
                    class="message-bubble px-4 py-3 rounded-lg text-sm bg-muted text-foreground rounded-bl-none mb-2 flex items-center gap-2"
                  >
                    <span class="animate-pulse">💭</span>
                  ...
                  <!-- 图片消息 -->
                  <div
                    v-else-if="message.isImage && message.imageUrl"
          ...
          <!-- 心跳消息组 -->
          <template v-else-if="group.type === 'heartbeat'">
            <div class="w-full flex justify-center">
              <div class="flex items-center gap-2 bg-amber-500/10 text-amber-600 px-4 py-2 rounded-lg text-sm max-w-[80%]">
                <IconifyIcon icon="mdi:heart-pulse" class="text-amber-500" />
                <div class="flex flex-col">
                  <span class="font-medium">{{ group.messages[0].sender }}</span>
                  <span class="text-amber-600/80">{{ group.messages[0].content }}</span>
                </div>
                <span class="text-xs text-amber-500/60 ml-2">{{ formatTime(group.messages[0].timestamp) }}</span>
              </div>
            </div>
          </template>
        </div>
