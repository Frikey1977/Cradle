...
import { List, Avatar, Tag, Spin, Empty, Input } from "ant-design-vue";
            ...
            >
              <div class="flex items-start gap-3 w-full px-4 py-3">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div
                    v-if="item.mode || item.pattern"
                    class="text-xs text-muted-foreground/70 truncate"
                  >
                    {{ item.mode }}{{ item.mode && item.pattern ? ' · ' : '' }}{{ item.pattern }}
                  </div>
                </div>
              </div>
            </List.Item>
