...
为了从业务层面跟踪还原 LLM 调用的全过程，需要在请求和响应中增加追踪字段，实现：

- **来源识别**: 区分 LLM 调用是由 Agent、Orchestrator 还是 Executor 发起
- **业务关联**: 通过 worktaskId 串联整个业务过程
- **可读性**: 使用 agentName 和 contactName 便于识别和调试

### 9.2 追踪字段定义

```typescript
type LLMCallSource = "agent" | "orchestrator" | "executor";

interface LLMTrackingFields {
  source?: LLMCallSource;    // 调用来源
  agentName?: string;         // Agent 名称（e_name）
  worktaskId?: string;        // 业务关联 ID
  contactName?: string;       // 联系人名称（e_name）
}
```
...
```
AgentRuntime / Orchestrator / Executor
    │
    │  LLMRequest { source, agentName, worktaskId, contactName }
    ▼
LLMClient (Worker Process)
    │
    │  IPC: ChatRequestPayload { source, agentName, worktaskId, contactName }
    ▼
LLMServiceManager (Master Process)
    │
    │  RoutingTask { source, agentName, worktaskId, contactName }
    ▼
OpenAICompatibleAdapter
    │
    │  ChatCompletionRequest { source, agentName, worktaskId, contactName }
    ▼
LLMLogger
    │
    │  LLMLogEntry { source, agentName, worktaskId, contactName }
    ▼
日志文件 / 数据库
```
...
| 组件 | 职责 |
|------|------|
| AgentRuntime | 设置 source="agent"，传入 agentName 和 contactName |
| Orchestrator | 设置 source="orchestrator"，传入 agentName、worktaskId 和 contactName |
| Executor | 设置 source="executor"，传入 worktaskId |
| LLMClient | 透传追踪字段到 IPC 消息 |
| LLMServiceManager | 透传追踪字段到适配器 |
| OpenAICompatibleAdapter | 将追踪字段传递给日志记录器 |
| LLMLogger | 记录追踪字段到日志条目 |
...
1. **问题排查**: 通过 worktaskId 快速定位业务流程中的 LLM 调用
2. **性能分析**: 按 source 分组统计各组件的 LLM 调用情况
3. **成本归因**: 按 agentName 和 contactName 统计 LLM 使用成本
4. **审计追踪**: 完整记录谁在什么时间调用了哪个模型
