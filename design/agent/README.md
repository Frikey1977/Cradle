...
Agent 系统采用**三层架构**设计，实现职责分离和任务编排能力：

```
┌─────────────────────────────────────────────────────────────────┐
│                        Agent 层                                  │
│  职责：用户交互、意图识别、结果汇总                               │
│  特点：完整上下文、Skill 第一级披露、非阻塞运行                   │
└─────────────────────────────────────────────────────────────────┘
                              │ IPC
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
| 隔离层级 | 说明 | OpenClaw 映射 |
|---------|------|--------------|
| 会话级 | Agent + Contact 对话 | session |
| 任务级 | Worktask (Orchestrator) | SubAgent Run |
| 执行级 | Executor | SubAgent (leaf) |
...
| 文档 | 说明 |
|------|------|
| [ARCHITECTURE.md](./ARCHITECTURE.md) | Agent 系统整体架构（三层架构） |
| [runtime.md](./runtime.md) | Agent 运行时层详细设计 |
| [orchestrator.md](./orchestrator.md) | Orchestrator 层设计 |
| [executor.md](./executor.md) | Executor 层设计 |
| [worktask.md](./worktask.md) | Worktask 任务隔离设计 |
