...
| 12 | mode | VARCHAR | 20 | YES | 'exclusive' | 服务模式（代码字典配置：organization.agents.mode） |
| 13 | pattern | VARCHAR | 50 | NO | NULL | 服务模式（代码字典配置：organization.agents.pattern） |
| 14 | config | JSON | - | NO | NULL | 模型配置 + 运行时配置 |
| 15 | profile | JSON | - | NO | NULL | 个人事实与偏好数据 |
| 16 | avatar | VARCHAR | 500 | NO | - | Agent 头像 |
| 17 | heartbeat | JSON | - | NO | NULL | 心跳配置（仅配置，运行时状态在内存） |
...
通过代码字典配置（organization.agents.mode），可选值：
...
通过代码字典配置（organization.agents.pattern），用于区分 Agent 的服务模式。
...
|----|------|------|
| chat | 对话模式 | 标准对话交互 |
| task | 任务模式 | 任务执行型 Agent |
| workflow | 工作流模式 | 工作流驱动型 Agent |
