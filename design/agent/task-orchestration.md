...
任务编排模块负责处理**复杂任务的分解与协调**，采用三层架构实现职责分离。
...
- 清晰的职责边界：对话、编排、执行分离
- 最小上下文原则：按需传递画像、记忆
- 渐进式 Skill 披露：Agent 看到元数据，Orchestrator 看到命令体
- 与 OpenClaw 兼容：复用 SubAgent 机制
...
| 角色 | 职责 | 特点 | 能力 |
|------|------|------|------|
| **Agent** | 用户交互、意图识别、结果汇总 | 全局视角，协调者 | 自然语言理解、任务分发、对话管理 |
| **Orchestrator** | 任务编排、ReAct 循环、Worktask 管理 | 任务专家 | 任务拆解、ReAct 循环、进度跟踪 |
| **Executor** | 执行具体任务 | 执行专家 | 工具调用、Skill 执行、进度汇报 |
...
const AgentConfig = {
  ...
  capabilities: ['对话', '意图识别', '结果汇总'],
  tools: [], // Agent 不直接调用工具
  skillDisclosure: 'level1', // metadata, worktask
  model: 'gpt-4',
};

// Orchestrator 配置 - ReAct 能力
const OrchestratorConfig = {
  role: '任务编排器',
  description: '负责复杂任务的编排与协调',
  capabilities: ['任务拆解', 'ReAct', 'Worktask管理'],
  skillDisclosure: 'level2', // body
  model: 'gpt-4',
  maxIterations: 20,
};
