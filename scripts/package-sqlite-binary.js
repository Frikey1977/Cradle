#!/usr/bin/env node
/**
 * better-sqlite3 预编译二进制文件打包脚本
 * 
 * 用途：将编译好的 better-sqlite3 二进制文件打包，便于部署到其他 Windows 环境
 * 
 * 使用方法：
 *   node scripts/package-sqlite-binary.js
 * 
 * 打包后的文件位于：prebuilt/win32-x64/
 */
...
const prebuiltDir = join(rootDir, 'prebuilt');
...
// 主函数
async function main() {
  logSection('better-sqlite3 预编译二进制文件打包工具');
  ...
  // 检查源文件是否存在
  const sourceFile = join(rootDir, 'node_modules', 'better-sqlite3', 'build', 'Release', 'better_sqlite3.node');
  
  if (!existsSync(sourceFile)) {
    log('✗ 错误: 找不到编译好的 better_sqlite3.node 文件', 'red');
    log(`  路径: ${sourceFile}\n`);
    log('请先运行 npm install 并确保 better-sqlite3 编译成功。\n', 'yellow');
    process.exit(1);
  }
  ...
  log(`✓ 找到编译好的二进制文件`, 'green');
  log(`  路径: ${sourceFile}`);
  log(`  大小: ${fileSizeKB} KB\n`);

  // 创建目标目录
  const targetDir = join(prebuiltDir, `${platform}-${arch}`);
  ...
  log(`✓ 创建版本信息: ${versionFile}\n`, 'green');

  // 创建部署脚本
  createDeployScript(platform, arch);
  ...
  log(`  📦 ${targetFile}`);
  log(`  📋 ${versionFile}`);
  log(`  🚀 ${join(prebuiltDir, 'install-sqlite.js')}\n`);

  log('部署说明：', 'bright');
  log('  1. 将整个 prebuilt/ 目录提交到 Git 或打包分发');
  log('  2. 在目标环境运行: node prebuilt/install-sqlite.js');
  log('  3. 脚本会自动将二进制文件复制到正确位置\n');
}

// 获取 better-sqlite3 版本
function getBetterSqlite3Version() {
  try {
    const packageJsonPath = join(rootDir, 'node_modules', 'better-sqlite3', 'package.json');
...
// 创建部署脚本
function createDeployScript(platform, arch) {
...
`;
  ...
  const scriptPath = join(prebuiltDir, 'install-sqlite.js');
  writeFileSync(scriptPath, scriptContent);
  log(`✓ 创建部署脚本: ${scriptPath}\n`, 'green');
}
